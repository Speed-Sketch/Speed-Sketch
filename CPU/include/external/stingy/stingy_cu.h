...
class StingyCU:public Sketch {
private:
	char clk, hash_size, *counter;
...
public:
	...
	StingyCU(uint32_t _w, uint32_t _d, uint32_t _hash_seed = 1000) {
		clk		= 0;
		d             	= _d;
		hash_size	= (d == 2) ? 0x20 : 0x40;
		// hash_func	= (d == 2) ? MurmurHash32 : MurmurHash64B;
		bias_range	= Min((hash_size - ceil(log2(_w))) / (d - 1),0X8);
	...
	void Insert(const char* str) {
		char preclk = clk;
		clk = (clk + 1) & THR;
		uint64_t hash_value = MurmurHash3_x86_32(str, KEY_LEN, hash_seed);
		uint32_t index = ((hash_value & index_mask) << bias_range);
		int32_t tmp = index - w;
		index = tmp < 0?index : tmp;
		hash_value >>= index_range;
		uint32_t ans = UINT32_MAX;
		int address[DMAX]={},result[DMAX]={};//d
		for(char i = 0;i < d;i++){
			prefch[preclk][i] = index+ (i << bias_range) + (hash_value & bias_mask);
			__builtin_prefetch(counter + prefch[preclk][i], 1);
			__builtin_prefetch(counter + (prefch[preclk][i] | 1), 0);
			hash_value >>= bias_range;
			int tmp = counter[prefch[clk][i]] & 0x3f;
			if(ans >= tmp)tmp += Calculate(prefch[clk][i] | 1)<< 6;
			if(ans >= tmp){
				address[i] = prefch[clk][i];
				result[i] = tmp;
				ans = tmp;
			}
		}
